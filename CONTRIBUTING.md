# Contributing to Straps

Thanks for helping keep AI on the rails. Straps is intentionally **zero-dependency** — the
...
node scripts/straps.mjs audit      # validate the sample library against the DS
node scripts/straps.mjs tokens     # regenerate src/styles/tokens.css from tokens.json
```

CI (`.github/workflows/ci.yml`) runs the tests + audit on Node 18/20/22 and fails if
`src/styles/tokens.css` is stale. Run `node scripts/straps.mjs tokens` and commit before pushing.

## Where things live

| Path | What |
|---|---|
| `scripts/straps.mjs` | CLI entry (validate/audit/check/import/tokens) |
...
`.straps/source/` — keep real/brand design systems there and regenerate artifacts with
`node scripts/straps.mjs import <dir>`.
