...
# Strap Bind — tokens, not literals
...
## Preconditions
`.strap/tokens.json` must be populated (run **strap-preflight** if not).

## Binding workflow
1. **Before writing**, for each value resolve it against `.strap/tokens.json`:
...
```bash
node -e "const t=require('./.strap/tokens.json');console.log(JSON.stringify(t,null,2))"
```
Match observed hex to `colors`, px to `spacing`/`radius`, family to `fonts`.

## Post-write QA (this is the enforcement)
After any edit, the **PostToolUse hook** runs `strap check` automatically and **blocks** writes
that contain off-spec `error`-level values, returning the exact line + the token to use. You can
also run it yourself:
```bash
node "$CLAUDE_PROJECT_DIR/scripts/strap.mjs" validate <files>   # or: audit (whole project)
```
...
3. Re-apply the edit. Do not silence the rule to get past it; fix the value or add a real token
   to `.strap/tokens.json` (and sync it to Figma).

Severity is configured in `strap.config.json` (`rules`): `rawHex`/`rawRgb`/`rawFont` are errors
