...
# Straps Intake — references become briefs, not pixel copies
...
## Preconditions
Best run after **straps-preflight** so tokens/registry exist to map against. If they don't, note
...
2. **Map to the Design System (the key step).** For every observed attribute, resolve to an
   ...
   - Each color → nearest token in `.straps/tokens.json` `colors` (note the delta).
   - Each spacing/size → nearest `spacing`/`radius` step.
   - Each font → a `fonts` entry (or flag "not in type system").
   - Each UI element → a `.straps/registry.json` component + variant (or "new component needed").
...
Write a structured brief (and offer to save it under `.straps/briefs/<name>.md`):
...
This brief is the contract for **straps-compose** (build the components) and **straps-bind**
